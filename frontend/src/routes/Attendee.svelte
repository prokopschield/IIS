<script>
	...
	let newActivity = {
		name: "Snorchling",
		description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Et odio pellentesque diam volutpat commodo sed. Et netus et malesuada fames ac turpis. Ut ornare lectus sit amet",
		date: "12.2.2021",
		time: "23:00"
	};
	...
	<div class="flex-auto p-4 ">
		<Carrousel>
			{#each upcomingActivities as upcomingActivity, index}
				{#if index === 0}
					<CarrouselCard activityTitle={upcomingActivity.name} cardId={index} previousCardId={upcomingActivities.length-1} nextCardId={index+1}>
				...
				{:else if index === (upcomingActivities.length - 1)}
					<CarrouselCard cardId={index} previousCardId={index-1} nextCardId={0}>
						<p class="mb-2">{upcomingActivity.description}</p>
						<p class="mb-2">{upcomingActivity.date}</p>
						<p class="mb-2">{upcomingActivity.time}</p>
					</CarrouselCard>
				{:else}
					<CarrouselCard cardId={index} previousCardId={index-1} nextCardId={index+1}>
						<p class="mb-2">{upcomingActivity.description}</p>
						<p class="mb-2">{upcomingActivity.date}</p>
						<p class="mb-2">{upcomingActivity.time}</p>
					</CarrouselCard>
	...
	<div class="flex-auto p-4 ">
		<Carrousel>
			{#each scoredActivities as scoredActivity, index}
				{#if index === 0}
					<CarrouselCard cardId={index} previousCardId={upcomingActivities.length-1} nextCardId={index+1}>
